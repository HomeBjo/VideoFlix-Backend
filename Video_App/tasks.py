...
def convert_video_480p(source):
    try:
        ...
        file_name, _ = os.path.splitext(source)
        print(f"failed to convert: {file_name} into 480p")  
...
def convert_video_720p(source):
    try:
        ...
        linux_target = "/mnt/" + target.replace('\\', '/').replace('C:', 'c')
        print("linux_source is ", linux_source)
        cmd = 'ffmpeg -i "{}" -s hd720 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(linux_source, linux_target)
        run = subprocess.run(cmd, capture_output=True, shell=True)
    except:
        file_name, _ = os.path.splitext(source)
        print(f"failed to convert: {file_name} into 720p") 